...
class FuelStation extends Building{ //Class FuelStation of the superclass Building
   ...
   */
  FuelStation(int x, int y, int wide, int high){
    super(x,y,wide,high); //Call the superclass constructor 
  }
  
  /*
   * doAction
   * This method refills the fuel of the player. Returns nothing
   * @param The player object
   */
  public void doAction(PlayerDrill player){
    player.reFuel(); //Call the repair health method in the player object
  }
  ...
  public void loadSprites() { 
    try { //Try catch to see if the image exists
      image = ImageIO.read(new File("fuel.png")); //Find and set the image
    } catch(Exception e) { 
